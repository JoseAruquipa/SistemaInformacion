...
###NOMBRE DE LA VISTACOMPRAS
def compras(request,compra_id=None):
    template_name="cmp/compras.html"
    prod=Producto.objects.filter(estado=True)#FILTRO PARA MOSTRAR LOS PRODUCTOS
    form_compras={}
    contexto={}
#
    if request.method=='GET':
...
###COMPROBAMOS IS EL LA COMPRA EXISTE
        if enc:
    ...
    if request.method=='POST':
        ...
        if not compra_id:
            ...
            if enc:
                ...
                enc.save()
##########obtener detalle de la compra333
        if not compra_id:
